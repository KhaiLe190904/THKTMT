.data
A: .word 1,2,3,-100,-200
messenger1: .asciiz "Max sum prefix is "
messenger2: .asciiz " with length: "
.text
main:   la $a0, A
        li $a1, 5
        j mspfx
        nop
continue: j mspfx_end
end_of_main:
mspfx: addi $s0,$zero,0 #initialize length in $s0 to 0
...
loop: add $t2,$t0,$t0 #put 2i in $t2
      ...
      j test #done?
mdfy:  addi $s0,$t0,1 #new max-sum prefix has length i+1
