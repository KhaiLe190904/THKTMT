...
ketqua: .asciiz "Ki tu dau tien co so lan xuat hien nhieu nhat trong xau la: "
space: .asciiz " "
.text 
input_data:
    li $v0, 4
...
find_max:
   li $t5, 32 # kí tự khoảng cách ' '
   sb $t5, 0($t3) # để gắn là kết thúc string_dif
   li $t0, -1 # i = -1
   li $t4, 0 # max_count = 0
   la $t3, string_dif
loop_stringdif:
   ...
   beq $s1, 32, end_main # gặp dấu khoảng cách ' ' kết thúc của string_dif thì print
   j loop_string
loop_string:
   addi $t1, $t1, 1 # j++
   add $s2, $t2, $t1 # $s2 = &string[j]
   ...
   j loop_string
compare_count:
   ble $t7, $t4, loop_stringdif # count <=  count_max
   move $t4, $t7 # update count_max
