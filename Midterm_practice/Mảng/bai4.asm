...
ketqua: .asciiz "Cap phan tu lien ke co tong nho nhat la: "
comma: .asciiz ", "
.text
main: 
   la $a0, messenger1
...
find_summin:
   ...
   lw $t2, 0($s0) # s0 = A[i]
   lw $t3, 4($s0) # s1 = A[i+1]
   add $s3, $t2, $t3  # s3 = A[i]+A[i+1]
   bgt $s3, $t0, find
   move $t0, $s3  # $t0 = min_new
   move $s4, $t2  # cập nhật cặp số mà có tổng là min
   move $s5, $t3 
   j find
print: 
   li $v0, 4
